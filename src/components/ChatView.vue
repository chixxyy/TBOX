  ...
  <div class="flex flex-col md:flex-row w-full h-full bg-[#05080f] text-slate-300">
                  ...
                  <div class="px-3 py-2 border-t border-slate-800/50 font-sans">
                    ...
                    <transition name="chat-news-expand">
                      <div v-if="expandedMessages.has(msg.id)" class="mt-3 pt-2 border-t border-slate-800/30 flex items-center gap-3">
                        ...
                        >
                          <svg v-if="translatingIds.has(msg.id)" class="w-3 h-3 animate-spin" viewBox="0 0 24 24" fill="none">
                          ...
                          <span>{{ translatedNews[msg.id] ? '原文' : '翻譯' }}</span>
                        </button>
    ...
    <transition name="fade">
      <div 
        ...
        class="fixed inset-0 z-[200] flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm"
      >
        <div @click.stop class="bg-[#111827] border border-red-900/50 rounded-xl p-6 w-full max-w-sm shadow-2xl text-center">
