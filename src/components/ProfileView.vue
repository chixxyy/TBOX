...
const updateTimeout = ref<any>(null) // 全域唯一計時器鎖
...
const confirmUpdatePassword = async () => {
  // 1. 防禦性檢查：防止連點造成的計時器錯亂
  if (!chatSession.value || passwordLoading.value) return
  ...
  try {
    ...
    let errorMsg = err.message || '請隨時再試'
...
<template>
  <div class="fixed inset-0 z-[150] flex items-center justify-center bg-[#05080f] overflow-hidden p-4 md:p-10">
    ...
    <!-- Profile Container (Centered Card with Vertical Margins) -->
    <div class="relative z-10 w-full max-w-[800px] h-full max-h-[92vh] flex flex-col items-center">
      
      <!-- Profile Header (Logo & Identity) -->
      <div class="w-full mb-4 md:mb-8 flex flex-row items-center justify-center gap-3 md:gap-6 animate-in fade-in duration-700 relative shrink-0">
        ...
        <!-- Close Button (Absolute on Card Top) -->
        <button 
          @click="handleLoginSuccess" 
          class="absolute right-0 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full border border-white/5 bg-white/5 flex items-center justify-center text-slate-400 hover:text-white transition-all shadow-lg"
        >
      ...
      <!-- Main Container Card -->
      <div class="w-full flex-1 bg-slate-900/60 backdrop-blur-3xl border border-white/5 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-[0_40px_120px_-20px_rgba(0,0,0,0.9)] flex flex-col relative">
        
        <!-- Tab Navigation (Fixed) -->
        <nav class="flex border-b border-white/5 bg-slate-950/30 shrink-0">
          <button 
            @click="activeSettingsTab = 'basic'"
            class="flex-1 py-4 md:py-6 text-[11px] md:text-xs font-black tracking-[0.2em] uppercase transition-all flex items-center justify-center gap-3 relative overflow-hidden group"
            :class="activeSettingsTab === 'basic' ? 'text-blue-400 bg-blue-500/5' : 'text-slate-500 hover:text-slate-300'"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 md:h-4 md:w-4 transition-transform group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
            基本設定
            <div v-if="activeSettingsTab === 'basic'" class="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-500 shadow-[0_0_10px_#3b82f6]"></div>
          </button>
          <button 
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 md:h-4 md:w-4 transition-transform group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
            安全維護
            <div v-if="activeSettingsTab === 'security'" class="absolute bottom-0 left-0 right-0 h-0.5 bg-orange-500 shadow-[0_0_10px_#f97316]"></div>
          </button>
        </nav>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-10">
          <transition name="tab-fade" mode="out-in">
            <!-- 1. Basic Info Tab -->
            <div v-if="activeSettingsTab === 'basic'" key="basic" class="space-y-8 md:space-y-12">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-12 lg:gap-16 items-center">
                <!-- Avatar Display Section -->
                <div class="flex flex-col items-center justify-center gap-6 md:border-r border-white/5 md:pr-10 lg:pr-14 min-h-0 md:min-h-[340px]">
                  ...
                  <div class="text-center w-full max-w-[280px]">
                    <div class="text-[11px] md:text-sm font-black text-slate-500 tracking-[0.2em] uppercase mb-4">預覽配色效果</div>
                    <div class="grid grid-cols-5 gap-3 p-4 bg-slate-950/40 border border-white/5 rounded-[1.8rem]">
                      <button 
                        v-for="color in avatarColors" 
                        :key="color.hex" 
                        @click="avatarBgColor = color.hex" 
                        class="w-7 h-7 md:w-8 md:h-8 rounded-full border md:border-2 transition-all hover:scale-125 active:scale-90" 
                        :style="{ backgroundColor: `#${color.hex}` }" 
                        :class="avatarBgColor === color.hex ? 'border-white ring-4 ring-blue-500/20 shadow-xl' : 'border-transparent opacity-50 hover:opacity-100'"
                      ></button>
                    </div>
                  </div>
                </div>
                ...
                <div class="space-y-8 md:pl-6">
                  <div class="space-y-4">
                    ...
                    <input v-model="nickname" type="text" placeholder="顯示於聊天室與排行榜" class="w-full bg-slate-950/40 border border-white/5 rounded-2xl px-6 py-4 md:py-5 text-white focus:outline-none focus:border-blue-500/50 transition-all font-bold placeholder:text-slate-800 text-base md:text-lg" />
                  </div>
                  <div class="space-y-4">
                    <label class="flex items-center gap-3 text-xs md:text-sm font-black text-blue-400 uppercase tracking-[0.3em] ml-1">
                      <div class="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-blue-500"></div>
                    ...
                    <textarea v-model="bio" :rows="windowWidth < 768 ? 3 : 5" placeholder="分享您的交易理念..." class="w-full bg-slate-950/40 border border-white/5 rounded-2xl px-6 py-5 md:py-6 text-white focus:outline-none focus:border-blue-500/50 transition-all text-sm md:text-base resize-none custom-scrollbar placeholder:text-slate-800"></textarea>
                  </div>
                </div>
              </div>
            </div>
            ...
            <!-- 2. Security Tab -->
            <div v-else key="security" class="space-y-8 md:space-y-12">
              <!-- Email badge -->
              <div class="bg-slate-950/40 border border-white/5 rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden group">
                ...
                <div class="flex items-center gap-6 relative z-10 w-full md:w-auto">
                  <div class="w-16 h-16 md:w-20 md:h-20 rounded-2xl md:rounded-3xl bg-slate-900 border border-white/5 flex items-center justify-center text-slate-400 group-hover:scale-105 transition-transform">
                  ...
                  <div class="min-w-0 flex-1">
                    ...
                    <p class="text-base md:text-2xl font-bold text-white tracking-tight break-all">{{ chatSession?.user.email }}</p>
                  </div>
                </div>
                <div class="flex items-center gap-3 px-5 py-3 md:px-6 md:py-3 bg-green-500/10 border border-green-500/20 rounded-full w-fit shrink-0">
                  <div class="w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse"></div>
                  <span class="text-[10px] md:text-xs font-black text-green-500 uppercase tracking-widest">已受安全保護</span>
                </div>
              </div>
              ...
              <div class="space-y-10">
                <div class="flex items-center gap-4">
                  <div class="w-2 h-8 bg-orange-500 rounded-full shadow-[0_0_15px_rgba(249,115,22,0.4)]"></div>
        ...
        <!-- STICKY FOOTER ACTION AREA (Boundary Definition) -->
        <div class="p-4 md:p-8 bg-slate-950/40 border-t border-white/5 backdrop-blur-2xl shrink-0">
          <div v-if="activeSettingsTab === 'basic'" class="animate-in fade-in zoom-in-95 duration-300">
            <button @click="handleSave" :disabled="loading" class="w-full bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white font-black py-4.5 md:py-5 rounded-xl md:rounded-2xl shadow-2xl shadow-blue-600/20 active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-3 text-sm md:text-base tracking-widest uppercase">
              ...
              {{ loading ? '更新程序執行中' : '儲存個人檔案變更' }}
            </button>
          </div>
          <div v-else class="animate-in fade-in zoom-in-95 duration-300">
            <button @click="handleUpdatePassword" :disabled="passwordLoading" class="w-full bg-slate-900/50 hover:bg-orange-600/10 border border-white/5 hover:border-orange-500/50 text-slate-400 hover:text-orange-400 font-black py-4.5 md:py-6 rounded-xl md:rounded-2xl transition-all disabled:opacity-50 flex items-center justify-center gap-4 text-xs tracking-[0.2em] uppercase">
              <svg v-if="passwordLoading" class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
              啟動安全密碼更新程序
            </button>
          </div>
        </div>
    ...
    <!-- HIGH-ALERT Security Confirmation Modal (Keep as is) -->
    <transition name="notice">
      <div v-if="showPasswordConfirm" class="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-slate-950/95 backdrop-blur-3xl">
        <div class="w-full max-w-[360px] md:max-w-[400px] bg-[#0a0606] border border-red-500/30 rounded-[2.5rem] overflow-hidden p-8 md:p-10 relative">
          <div class="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-red-600 to-red-600"></div>
          <div class="relative z-10 text-center">
            <div class="w-16 h-16 md:w-20 md:h-20 bg-red-950/30 border border-red-500/20 rounded-2xl md:rounded-3xl flex items-center justify-center text-red-500 mx-auto mb-8">
            ...
            <div class="text-[9px] md:text-[10px] text-red-500/60 font-mono mb-8 uppercase tracking-[0.3em]">SECURITY_AUTH_CONFIRM</div>
            
            <p class="text-sm text-slate-400 mb-10 leading-relaxed">
              您正在發起帳戶權限重設請求。
            </p>
            ...
            <div class="flex flex-col gap-4">
              <button 
                ...
                class="w-full py-5 bg-red-600 text-white font-black rounded-2xl transition-all shadow-xl active:scale-95 text-xs tracking-widest"
              >
                確認變更
              </button>
              <button 
                ...
                :disabled="passwordLoading"
                class="w-full py-5 bg-white/5 text-slate-500 font-bold rounded-2xl text-xs tracking-widest"
              >
                取消
              </button>
            </div>
